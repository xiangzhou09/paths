...
out_all_draw <- foreach(i = seq(1, K), .combine = rbind, .packages = c("dplyr", "BART"), .errorhandling="remove") %dopar% {
...
out_draw <- out_all_draw[,1:5]
out2_draw <- out_all_draw[,6:10]

lower <- function(x) quantile(x, 0.025)
upper <- function(x) quantile(x, 0.975)

peace_decomp_draw <- data.frame(out_draw) %>%
...
peace_decomp2_draw <- data.frame(out2_draw) %>%
...
save(peace_decomp_draw, peace_decomp2_draw, file = "data/peace_bart_draw.RData")
