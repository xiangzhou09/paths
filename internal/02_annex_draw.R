...
out_all <- foreach(i = seq(1, K), .combine = rbind, .packages = c("dplyr", "BART"), .errorhandling="remove") %dopar% {
  ...
  mu_bart_AL_10 <- predict(bart_AL, AL10)[["prob.test.mean"]]
  mu_bart_AL_01 <- predict(bart_AL, AL01)[["prob.test.mean"]]

  bart_AL_10 <- rbinom(nrow(AL10), size = 1, prob = mu_bart_AL_10)
  bart_AL_01 <- rbinom(nrow(AL01), size = 1, prob = mu_bart_AL_01)
  ...
  # bart_ay_amy_any <- weighted.mean(bart_AL_10, w = ipw0) - mean(bart_A_0[!treat])
  # bart_ay_amy_any2 <- mean(bart_A_1[treat]) - weighted.mean(bart_AL_01, w = ipw1)

  bart_ay_amy_any <- weighted.mean(bart_AL_10, w = ipw0) - mean(bart_A_0)
  bart_ay_amy_any2 <- mean(bart_A_1) - weighted.mean(bart_AL_01, w = ipw1)

  # pure imputation

  bart_AL_supp <- pbart(x.train = X0, y.train = bart_AL_10, x.test = X)[["prob.test.mean"]]
  bart_AL_supp2 <- pbart(x.train = X1, y.train = bart_AL_01, x.test = X)[["prob.test.mean"]]

  bart_ay_amy_any_pi <- mean(bart_AL_supp) - mean(bart_A_0)
  bart_ay_amy_any2_pi <- mean(bart_A_1) - mean(bart_AL_supp2)
  ...
  mu_bart_ALM_10 <- predict(bart_ALM, ALM10)[["prob.test.mean"]]
  mu_bart_ALM_01 <- predict(bart_ALM, ALM01)[["prob.test.mean"]]

  bart_ALM_10 <- rbinom(nrow(ALM10), size = 1, prob = mu_bart_ALM_10)
  bart_ALM_01 <- rbinom(nrow(ALM01), size = 1, prob = mu_bart_ALM_01)

  # bart_ay_any <- weighted.mean(bart_ALM_10 - bart_A_0[!treat], w = ipw0)
  # bart_ay_any2 <- weighted.mean(bart_A_1[treat] - bart_ALM_01, w = ipw1)

  # bart_ay_any <- weighted.mean(bart_ALM_10, w = ipw0) - mean(bart_A_0[!treat])
  # bart_ay_any2 <- mean(bart_A_1[treat]) - weighted.mean(bart_ALM_01, w = ipw1)

  bart_ay_any <- weighted.mean(bart_ALM_10, w = ipw0) - mean(bart_A_0)
  bart_ay_any2 <- mean(bart_A_1) - weighted.mean(bart_ALM_01, w = ipw1)

  # pure imputation

  bart_ALM_supp <- pbart(x.train = X0, y.train = bart_ALM_10, x.test = X)[["prob.test.mean"]]
  bart_ALM_supp2 <- pbart(x.train = X1, y.train = bart_ALM_01, x.test = X)[["prob.test.mean"]]

  bart_ay_any_pi <- mean(bart_ALM_supp) - mean(bart_A_0)
  bart_ay_any2_pi <- mean(bart_A_1) - mean(bart_ALM_supp2)
  ...
  mu_bart_ALMN_10 <- predict(bart_ALMN, ALMN10)[["prob.test.mean"]]
  mu_bart_ALMN_01 <- predict(bart_ALMN, ALMN01)[["prob.test.mean"]]

  bart_ALMN_10 <- rbinom(nrow(ALMN10), size = 1, prob = mu_bart_ALMN_10)
  bart_ALMN_01 <- rbinom(nrow(ALMN01), size = 1, prob = mu_bart_ALMN_01)

  # bart_ay <- weighted.mean(bart_ALMN_10 - bart_A_0[!treat], w = ipw0)
  # bart_ay2 <- weighted.mean(bart_A_1[treat] - bart_ALMN_01, w = ipw1)

  # bart_ay <- weighted.mean(bart_ALMN_10, w = ipw0) - mean(bart_A_0[!treat])
  # bart_ay2 <- mean(bart_A_1[treat]) - weighted.mean(bart_ALMN_01, w = ipw1)

  bart_ay <- weighted.mean(bart_ALMN_10, w = ipw0) - mean(bart_A_0)
  bart_ay2 <- mean(bart_A_1) - weighted.mean(bart_ALMN_01, w = ipw1)

  # pure imputation

  bart_ALMN_supp <- pbart(x.train = X0, y.train = bart_ALMN_10, x.test = X)[["prob.test.mean"]]
  bart_ALMN_supp2 <- pbart(x.train = X1, y.train = bart_ALMN_01, x.test = X)[["prob.test.mean"]]

  # bart_ay_pi <- mean(bart_ALMN_supp - bart_A_0[!treat])
  # bart_ay2_pi <- mean(bart_A_1[treat] - bart_ALMN_supp2)

  bart_ay_pi <- mean(bart_ALMN_supp) - mean(bart_A_0)
  bart_ay2_pi <- mean(bart_A_1) - mean(bart_ALMN_supp2)
...
tatar_decomp_draw <- data.frame(out) %>%
...
tatar_decomp2_draw <- data.frame(out2) %>%
...
tatar_decomp_pi_draw <- data.frame(out_pi) %>%
  `names<-`(c("te", "ay", "aly", "amy", "any")) %>%
  summarise_all(list(est = mean, lower = lower, upper = upper)) %>%
  gather(measure, value) %>%
  separate(measure, c("estimand", "measure")) %>%
...
tatar_decomp2_pi_draw <- data.frame(out2_pi) %>%
  `names<-`(c("te", "ay2", "aly2", "amy2", "any2")) %>%
  summarise_all(list(est = mean, lower = lower, upper = upper)) %>%
  gather(measure, value) %>%
  separate(measure, c("estimand", "measure")) %>%
...
save(tatar_decomp_draw, tatar_decomp2_draw, tatar_decomp_pi_draw, tatar_decomp2_pi_draw, file = "data/annex_bart_draw.RData")
